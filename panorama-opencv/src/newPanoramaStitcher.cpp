...
static void printUsage()
{
    cout <<
      ...
      "  --match_type (pair_wise|adjacent). The default is pair_wise"
...
static int parseCmdArgs(int argc, char** argv)
{
	...
	else if (string(argv[i]) == "--match_type")
	  {
	    match_type = argv[i + 1];
	    i++;
	  }
...
int main(int argc, char* argv[])
{
    ...
    if (match_type == "pair_wise"){
      (*matcher)(features, pairwise_matches);
    }
    else{
      const int num_images = static_cast<int>(features.size());
      UMat _mask = UMat::zeros(num_images, num_images, CV_8U);
      Mat_<uchar> mask = _mask.getMat(ACCESS_WRITE);

      for (int i=0; i < num_images-1;++i){	
	mask(i,i+1) = 1;
      }

      (*matcher)(features, pairwise_matches, _mask);
    }
    
    matcher->collectGarbage();
    
    LOGLN("Matching, time: " << ((getTickCount() - t) / getTickFrequency()) << " sec");
